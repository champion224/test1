...
function createDayCell(dateObj, isOtherMonth) {
  ...
  // 데이터 매칭
  const matched = data.map((item, idx) => ({ ...item, idx }))
...
function openDetailModal(dateStr) {
  ...
  for (const category in grouped) {
    ...
    grouped[category].forEach(item => {
      ...
      li.textContent = `${item.name} (${item.qty}${item.qtyUnit}) ${item.expiry} ${item.note}`.trim();
...
function parseAndAddCsv(csvText) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const item = {
      category: cols[idxCategory] ? cols[idxCategory].replace(/^"|"$/g, '') : '',
...
downloadTemplateBtn.addEventListener('click', () => {
  const BOM = "\uFEFF"; // 한글 깨짐 방지용 BOM
  const header = ['분류', '상품명', '수량', '단위', '유통기한', '비고'];
  const exampleRow = ['카페', '아메리카노', '10', 'EA', '2025-08-05', '시원한 음료'];
  const noteLines = [
    '# 주의사항:',
    '# 1. "수량"은 숫자만 입력하세요.',
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  switch (period) {
    ...
    console.log('input values set ->', exportStartDate.value, exportEndDate.value);
  }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', function () {
      ...
      const csvContent = '분류,상품명,수량,단위,유통기한,비고\n카페,아메리카노,10,EA,2025-12-31,테스트\n';
